...
public class VpnStateService extends Service
{
	...
	private final Map<VpnProfile, Long> connectionList = new HashMap<>();

	/**
	 * 连接失败时需断开vpn的连接，否则会导致网络不可用
	 */
	public void closeBlockingInError() {
		Context context = getApplicationContext();
		Intent intent = new Intent(context, CharonVpnService.class);
		intent.setAction(CharonVpnService.CLOSE_BLOCKING_ACTION);
	 ...
	 * @param nextProfile
	 */
	public void startConnect(Bundle profileInfo, boolean nextProfile) {
		isConnectFinish = false;
		connect(profileInfo, nextProfile);
	}
	 ...
	 * @param vpnProfile
	 */
	private void beforeStartConnection(VpnProfile vpnProfile) {
		connectionList.put(vpnProfile, System.currentTimeMillis());
	}
	 ...
	 * @param isSuccess
	 */
	private void afterConnectDeal(VpnProfile profile, boolean isSuccess) {
		long endTime = System.currentTimeMillis();
		Long startTime = connectionList.remove(profile);
		if (startTime == null) {
			startTime = 0L;
		}
	 ...
	 * @param nextProfile
	 */
	public void connect(Bundle profileInfo, boolean nextProfile)
	{
		...
		if (nextProfile)
		{
			VpnProfile profile = VpnProfileDataSource.getInstance().getNextVpnProfile();
			if (profile != null) {
			...
			public Boolean call() throws Exception
			{
				if (VpnStateService.this.mError != error)
				{
					if (VpnStateService.this.mError == ErrorState.NO_ERROR)
					{
						...
						if (VpnProfileDataSource.getInstance().hasNextProfile()) {
							setRetryTimer(error);
							return false;
						} else {
							isConnectFinish = true;
